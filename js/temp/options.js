function updateTempOptions() {
	...
	tmp.options.save = function (sav = player, force = false) {
		...
		if (
			(all.includes(null) || all[sav.savePos - 1] === undefined || all[sav.savePos - 1].savePos == sav.savePos) &&
			all.length >= sav.savePos
		)
	...
	tmp.options.loads = function () {
		...
		for (let x = 0; x < all.length; x++) {
			...
			els[x] = {
				...
				info: tmp.options.getInfo(all[x]),
				onclick1: "tmp.options.setSave(&quot;" + btoa(JSON.stringify(all[x])) + "&quot;, true)",
				txt1: "Load",
				onclick2: "tmp.options.deleteSave(" + x + ")",
				txt2: "Delete",
				buttons: 2
			};
		}
