function updateHTML() {
	...
	for (let i = 1; i <= PTH_AMT; i++) {
		...
		tmp.el["pth" + i].setHTML(
			PTH_UPGS[i].desc +
				...
				(player.pathogens.upgrades[i].gte(tmp.pathogens.sc[i]) ? "<span class='sc'>(softcapped)</span>" : "") +
				"<br>Cost: " +
	...
	tmp.el.darkFluid.setHTML(
		"Dark Fluid<br>Amount: " +
			...
			"<br>Effect: Scaled Rocket Fuel scaling starts " +
	...
	tmp.el.ascPower.setHTML(
		"Ascension Power: <span style='font-size: 25px; color: red;'>" + showNum(player.inf.ascension.power) + "</span>"
	);
		...
		player.inf.pantheon.purge.active
			? "Exit Purge run" +
					(tmp.inf.pantheon.purgeGain.gt(0)
						...
						: ". You need " + formatDistance(tmp.inf.pantheon.purgeNext) + " to gain more Purge Power.")
			: "Start Purge run"
	);
