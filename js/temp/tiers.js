function updateTempTiers() {
	...
	if (player.tr.upgrades.includes(20) && tmp.modes.extreme.active) tmp.tiers.fp = tmp.tiers.fp.times(player.rankCheap.plus(1).log10().plus(1).log10().plus(1))
	...
	if (tmp.scaling.active("tier", player.tier.max(tmp.tiers.bulk), "scaled")) {
		...
		tmp.tiers.bulk = player.rank.sub(tmp.tiers.bc).max(0).sqrt().times(tmp.tiers.fp).times(tmp.scalings.scaled.tier.pow(exp.sub(1))).pow(exp.pow(-1)).add(1).floor()
	}
	if (tmp.scaling.active("tier", player.tier.max(tmp.tiers.bulk), "superscaled")) {
		...
		tmp.tiers.bulk = player.rank.sub(tmp.tiers.bc).max(0).sqrt().times(tmp.tiers.fp).times(tmp.scalings.scaled.tier.pow(exp.sub(1))).pow(exp.pow(-1)).times(tmp.scalings.superscaled.tier.pow(exp2.sub(1))).pow(exp2.pow(-1)).add(1).floor()
	}
	if (tmp.scaling.active("tier", player.tier.max(tmp.tiers.bulk), "hyper")) {
		...
		tmp.tiers.bulk = player.rank.sub(tmp.tiers.bc).max(0).sqrt().times(tmp.tiers.fp).times(tmp.scalings.scaled.tier.pow(exp.sub(1))).pow(exp.pow(-1)).times(tmp.scalings.superscaled.tier.pow(exp2.sub(1))).pow(exp2.pow(-1)).div(tmp.scalings.hyper.tier).max(1).logBase(base3).add(tmp.scalings.hyper.tier).add(1).floor()
	}
	if (tmp.scaling.active("tier", player.tier.max(tmp.tiers.bulk), "atomic")) {
		...
		let power = tmp.scalingPower.scaled.tier
		let exp = ExpantaNum.pow(2, power)
		tmp.tiers.req = new ExpantaNum(tmp.tiers.bc).plus(((ExpantaNum.pow(base3, player.tier.pow(exp4).div(tmp.scalings.atomic.tier.pow(exp4.sub(1))).sub(tmp.scalings.hyper.tier)).times(tmp.scalings.hyper.tier).pow(exp2).div(tmp.scalings.superscaled.tier.pow(exp2.sub(1)))).pow(exp).div(tmp.scalings.scaled.tier.pow(exp.sub(1)))).div(tmp.tiers.fp).pow(2))
		tmp.tiers.bulk = player.rank.sub(tmp.tiers.bc).max(0).sqrt().times(tmp.tiers.fp).times(tmp.scalings.scaled.tier.pow(exp.sub(1))).pow(exp.pow(-1)).times(tmp.scalings.superscaled.tier.pow(exp2.sub(1))).pow(exp2.pow(-1)).div(tmp.scalings.hyper.tier).max(1).logBase(base3).add(tmp.scalings.hyper.tier).times(tmp.scalings.atomic.tier.pow(exp4.sub(1))).pow(exp4.pow(-1)).add(1).floor()
	}
