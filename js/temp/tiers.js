function updateTempTiers() {
	...
	if (tmp.scaling.active("tier", player.tier.max(tmp.tiers.bulk), "hyper")) {
		...
		let power = tmp.scalingPower.scaled.tier
		let exp = ExpantaNum.pow(2, power)
		tmp.tiers.req = new ExpantaNum(tmp.tiers.bc).plus(((ExpantaNum.pow(base3, player.tier.div(tmp.scalings.hyper.tier).sub(1)).times(tmp.scalings.hyper.tier).pow(exp2).div(tmp.scalings.superscaled.tier.pow(exp2.sub(1)))).pow(exp).div(tmp.scalings.scaled.tier.pow(exp.sub(1)))).div(tmp.tiers.fp).pow(2))
		tmp.tiers.bulk = player.rank.sub(tmp.tiers.bc).max(0).sqrt().times(tmp.tiers.fp).times(tmp.scalings.scaled.tier.pow(exp.sub(1))).pow(exp.pow(-1)).times(tmp.scalings.superscaled.tier.pow(exp2.sub(1))).pow(exp2.pow(-1)).div(tmp.scalings.hyper.tier).max(1).logBase(base3).add(1).times(tmp.scalings.hyper.tier).add(1)
	}
