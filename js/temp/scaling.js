function updateTempScaling() {
	...
	for (let t=0;t<Object.keys(SCALING_STARTS).length;t++) {
		...
		for (let p=0;p<Object.keys(SCALING_STARTS[name]).length;p++) {
			let name2 = Object.keys(SCALING_STARTS[name])[p]
			if (next!==undefined) if (tmp.scalings[next][name2]!==undefined) tmp.scalings[name][name2] = ExpantaNum.min(tmp.scalings[name][name2], tmp.scalings[next][name2])
		}
	}
	...
	if (tmp.pathogens) {
		...
		tmp.scalingPower.superscaled.rank = tmp.scalingPower.superscaled.rank.times(ExpantaNum.sub(1, tmp.pathogens[14].eff))
		tmp.scalingPower.scaled.endorsements = tmp.scalingPower.scaled.endorsements.times(ExpantaNum.sub(1, tmp.pathogens[15].eff))
	}
	if (tmp.inf) {
		...
		if (tmp.inf.upgs.has("9;6")) tmp.scalingPower.superscaled.rank = tmp.scalingPower.superscaled.rank.times(ExpantaNum.sub(1, INF_UPGS.effects["9;6"]()))
		if (tmp.inf.upgs.has("8;6")) tmp.scalingPower.hyper.rank = tmp.scalingPower.hyper.rank.times(ExpantaNum.sub(1, INF_UPGS.effects["8;6"]()))
		...
		if (tmp.inf.upgs.has("9;6")) tmp.scalingPower.superscaled.tier = tmp.scalingPower.superscaled.tier.times(ExpantaNum.sub(1, INF_UPGS.effects["9;6"]()))
