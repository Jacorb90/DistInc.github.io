...
function tickWithoutTS(diff) {
	...
	if (tmp.ach[95].has && !tmp.nerfs.active("noRockets"))
		player.rockets = player.rockets.plus(tmp.nerfs.adjust(tmp.rockets.layer.gain, "rockets").times(diff));
	else if (tmp.collapse.hasMilestone(9) && !tmp.nerfs.active("noRockets"))
		player.rockets = player.rockets.plus(tmp.nerfs.adjust(tmp.rockets.layer.gain, "rockets").times(diff.div(100)));
	...
	if (tmp.ach[97].has && !tmp.nerfs.active("noLifeEssence"))
		player.collapse.lifeEssence = player.collapse.lifeEssence.plus(
			tmp.nerfs.adjust(player.collapse.cadavers.times(tmp.collapse.sacEff), "lifeEssence").times(diff)
		);
	else if (tmp.inf.upgs.has("5;3") && !tmp.nerfs.active("noLifeEssence"))
		player.collapse.lifeEssence = player.collapse.lifeEssence.plus(
			tmp.nerfs.adjust(player.collapse.cadavers.times(tmp.collapse.sacEff), "lifeEssence").times(diff.div(10))
		);
	if (tmp.ach[96].has && !tmp.nerfs.active("noCadavers"))
		player.collapse.cadavers = player.collapse.cadavers.plus(
			tmp.nerfs.adjust(tmp.collapse.layer.gain, "cadavers").times(diff)
		);
	else if (tmp.inf.upgs.has("2;4") && !tmp.nerfs.active("noCadavers"))
		player.collapse.cadavers = player.collapse.cadavers.plus(
			tmp.nerfs.adjust(tmp.collapse.layer.gain, "cadavers").times(diff.div(100))
		);
