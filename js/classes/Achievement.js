class Achievement {
	...
	parse(txt) {
		...
		if (txt.indexOf("showNum")!=-1) {
			...
			return txt.split("formatDistance")[0]+formatDistance(txt2.slice(1, txt2.indexOf(")")))+txt2.split(")")[1]
	...
	get reward() { return (ACH_DATA.rewards[this.name]!==undefined)?this.parse(ACH_DATA.rewards[this.name]):"" }
	
	get title() { return (ACH_DATA.names[this.name]!==undefined)?ACH_DATA.names[this.name]:"" }
	
	get desc() { return "<b>"+this.title+"</b><br>"+(ACH_DATA.descs[this.name]!==undefined?this.parse(ACH_DATA.descs[this.name]):"Not currently implemented.")+"<br>"+(this.reward===""?"":("Reward: "+this.reward)) }
